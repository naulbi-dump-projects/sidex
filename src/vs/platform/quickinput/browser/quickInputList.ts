...
class QuickPickItemElementRenderer extends BaseQuickInputListRenderer<QuickPickItemElement> {
	...
	renderElement(node: ITreeNode<QuickPickItemElement, void>, index: number, data: IQuickInputItemTemplateData): void {
		...
		if (buttons && buttons.length) {
			const { primary, secondary } = quickInputButtonsToActionArrays(buttons, 'quick-input-item', button =>
...
class QuickPickSeparatorElementRenderer extends BaseQuickInputListRenderer<QuickPickSeparatorElement> {
	...
	override renderElement(
		...
		const buttons = mainItem.buttons;
		if (buttons && buttons.length) {
			const { primary, secondary } = quickInputButtonsToActionArrays(buttons, 'quick-input-separator', button =>
...
export class QuickInputList extends Disposable {
	...
	readonly onChangedAllVisibleChecked: Event<boolean> = Event.fromObservable(
	...
	readonly onChangedCheckedElements: Event<IQuickPickItem[]> = Event.fromObservable(
	...
	constructor(
		...
		this._separatorRenderer = this._register(
			instantiationService.createInstance(QuickPickSeparatorElementRenderer, hoverDelegate, this.styles.toggle)
		);
		this._itemRenderer = this._register(
			instantiationService.createInstance(QuickPickItemElementRenderer, hoverDelegate, this.styles.toggle)
		);
	...
	private _registerHoverListeners() {
		const delayer = this._register(
			new ThrottledDelayer(
				typeof this.hoverDelegate.delay === 'function' ? this.hoverDelegate.delay() : this.hoverDelegate.delay
			)
		);
		this._register(
	...
	private _registerSeparatorActionShowingListeners() {
		...
		this._register(
			this._tree.onMouseOver(e => {
				const parent = e.element ? (this._tree.getParentElement(e.element) as QuickPickSeparatorElement) : null;
				for (const separator of this._separatorRenderer.visibleSeparators) {
					if (separator !== parent) {
						continue;
					}
		...
		// Filter by value (since we support icons in labels, use $(..) aware fuzzy matching)
			...
			this._itemElements.forEach(element => {
				...
				const descriptionHighlights = this.matchOnDescription
					? (matchesFuzzyIconAware(query, parseLabelWithIcons(element.saneDescription || '')) ?? undefined)
					: undefined;
				const detailHighlights = this.matchOnDetail
					? (matchesFuzzyIconAware(query, parseLabelWithIcons(element.saneDetail || '')) ?? undefined)
					: undefined;
